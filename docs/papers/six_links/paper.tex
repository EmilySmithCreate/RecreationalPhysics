...
\title{Curled tori with six and eight links: an additive ladder of directions, walls that double at every rung, a
...
per curled direction; the cheapest single move out of each rung, listed by brute force, doubles in cost from rung to
...
cheapest kind for every rung at the sizes used. Three things follow. The walls double from rung to rung: with six
links 16 then 36, with eight links 20, 40, 80. So the energy ties nothing together; each direction has its own wall,
...
\emph{Pre-registered as T33; running as this draft is written.} With eight links the author's two variants can be told
...
and the rests on them. This section will report what was measured.
...
Three exact facts and two measured ones. The ladder is additive at every number of links; the walls double from rung
to rung; flat space's own wall grows with the number of links faster than any rung's. The push that starts a change
is fixed with size in three dimensions as in two. And given that push, the directions open one at a time: the first
as a front, the second behind a wall the first release does not pay in a cold bath. The author's rule that the
