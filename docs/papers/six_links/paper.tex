...
push fixed with size, and directions that open one at a time}

\author{Emily Smith}
\email{emilysmithcreate@gmail.com}
\affiliation{Independent researcher, Charlottesville, Virginia, USA}

\date{DRAFT, 25 September 2026; the eight-link pattern test is running}
...
per curled direction; the cheapest single move out of each rung, listed by brute force, rises in cost from rung to
rung (with six links 16, 36; with eight, 20, 40, 80, at $\lambda = 1.25$), so nothing in the energy ties the
directions together; and the cheapest move out of flat space costs 32, 64 and 128 with four, six and eight links,
the same at every $\lambda$. Measured, pre-registered, sealed and energy-conserving: the push that starts the change
from a six-link torus with two directions curled is exactly 16 at every size from 192 to 512 vertices, nothing
below it ever starting the change and everything at or above it doing so; given that push, the first curled direction
opens completely in a third of the runs and partly in most of the rest, and the second never opens in a cold bath;
and a gas of fully curled 6-cubes at $\lambda = 1.10$ joins and opens two of its three directions in 21 of 24
replicas, without resting between them, but never into one space. The author's rule that the space directions curl
and open together predicted a cascade; in a torus they do not open together. The published six-link curve has not been reproduced, and every result here carries that caveat.
\end{abstract}
...
\cite{repo}. With six and eight links it can, and this paper poses it: exactly, by listing every single move out of
...
the published model. And the model has no time: a direction of a torus is a direction, and which one would be time
...
With $2D$ links per vertex, $S$ squares and $X = \sum_e (S_e - (2D - 2))_+$ surplus squares,
\begin{equation}
  H = 16\Big(\tfrac{D(D-1)}{2} N - S\Big) + 4\lambda X ,
\end{equation}
derived from the published definitions exactly as the four-link energy was \cite{repo}. The flat $D$-torus (every
...
Every switch the chain can propose out of each curled torus was listed and priced at $-16\,\Delta S + 4\lambda\,\Delta X$;
by translation symmetry one vertex serves as the first of the pair, and the cheapest moves are always among nearby
partners, which was checked against the full search on the six-link tori \cite{repo}. Table~\ref{tab:walls} gives the
...
and the second is the higher. The window of $\lambda$ in which a rung is stuck for now widens as more directions
are open (with eight links, 1.25 for the gas, 1.43, 1.67, 2.5 up the ladder). And the wall of flat space itself, the
cheapest move out of the perfect lattice, is 32, 64 and 128 with four, six and eight links, independent of $\lambda$:
with eight links it sits above every rung's wall, so a bath hot enough to pay the later rungs need not melt the flat
state, where with four links the sheet's wall (32) is only a little above the tube's (12). One correction to the
...
\subsection{The push is fixed with size}
...
sweeps: no replica left the start at 12, 14 or 15, and every replica left at 16 and above, at every size. The
threshold is exactly the wall of Table~\ref{tab:walls}, and it does not grow with the system, as the four-link push
of 12 did not \cite{paper1}.

\subsection{Given the push, one direction opens}
...
energy 1.86 to 1.95 per vertex against 2.0 at the start; the bath ends near 0.4 to 0.5 per store. The second
direction never opens in a cold bath, its wall of 36 being unpayable there. (An earlier draft said every run at
these baths reached the one-curled rung; the final states say a third did.) With $C = N/8$ the bath heats to 1 to 5 per
store and two of twelve replicas descend both rungs to a defective near-flat state (88 to 90\% of vertices at
$d = 3$, 3\% of squares lost), just under the pre-registered cleanliness bar; the rest end as mixtures. At
$\lambda = 1.10$ a spark of 26 (the wall is 25.6) buys one move, which neither heals nor grows in 100,000 sweeps.
No cell has a flat majority and no torus cell has a majority on the one-curled rung; the verdict, FIRST ONLY, is
carried by the letter by the gas cell below.
...
10,000 to 46,000 sweeps. This is further than the two-curled torus went,
because the gas's walls are lower (8, then the moves that join two cubes) than the torus's second wall (43 at
$\lambda = 1.10$).
...
was tested, because nothing opened. What the run does establish is a difference between the number of directions: with
four links the push equal to the cheapest first move (12) is exactly the push that starts the change, and with six
links at $\lambda = 1.25$ it starts the first direction in a third of the runs, but with eight links the change needs
more than its first move; its activation is the height of a pass several moves high. A scan of the push from 20 to
160 (T40, pre-registered) is the test that finds it and then reads the pattern.
...
on the log axis, was pre-registered and failed, and the same code's two-dimensional figure is not compressed. A
plain factor of two in the coupling matches the ordered side and the top of the transition (the crossings of 9 and
6 squares per vertex within 0.02 and 0.03 of the published values); what remains different is the hot tail, which the
published curve reaches a factor of 2.8 sooner in coupling, and the plateau (10.07 against our 11.4). Two readings are
open. The published graphs may carry triangles and pentagons, as the paper says its ground states can; against that,
the published hot end (1.09 squares per vertex) sits at our bipartite value (1.15), where such graphs would sit near
half of it. Or the published runs were short and out of equilibrium: a fast heating leg from a slowly cooled state
reproduces the published plateau and steepness in our code. The paper states no protocol, and the second reading is
the better supported. Until it is settled, everything above is a statement about our family and its kernel, validated on its
...
to rung; flat space's own wall grows with the number of links faster than any rung's. The push that starts a change
is fixed with size in three dimensions as in two. And given that push, a torus's directions do not open together: the
first opens fully in a third of the runs and partly in most of the rest, and the second sits behind a wall the first
release does not pay in a cold bath; the gas of cubes, where no direction is singled out, opens two of three without a
rest between them. The author's rule that the
directions curl and open together therefore describes an ingredient this energy does not contain, which the map now
says plainly; the eight-link test will say whether a singleton-then-three pattern, which she found natural on seeing
the six-link result, appears from the fully curled start. What would carry the question further is a term that
...
PREREGISTRATION T30, T32, T33, Gate C$'$; ASSUMPTIONS Q21, O41, O43, O46, O49, O50, O53, O54.
\end{thebibliography}
