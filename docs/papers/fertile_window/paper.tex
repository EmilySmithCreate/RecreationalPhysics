...
links the same ladder holds, the walls double from rung to rung, and the push that starts the change from a
two-curled six-link torus is exactly 16 at every size; given it, one direction opens and the second waits. A
...
listing every switch (Table~\ref{tab:dmap}). Three things are exact. The walls double from rung to rung at every
number of directions, so nothing in the energy ties the directions together. More directions widen the $\lambda$
