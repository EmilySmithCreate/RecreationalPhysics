...
\title{The curled torus burps: front-driven decompactification in combinatorial quantum gravity}
...
$4(\lambda - 1)$ per vertex above it. At $\lambda = 1.25$ the curled torus is metastable, and its
decompactification at fixed coupling proceeds by a memoryless nucleation followed by a single front: at half
conversion 98.8 to 99.8\% of vertices belong to one ordered state or the other, and the energy released is
exactly $\varepsilon = 4(\lambda-1)$ per vertex. The nucleation barrier is a single local move, and the
waiting time follows an Arrhenius law with nothing fitted. In a sealed (microcanonical) system a seed equal to
the activation energy, 12 units, starts the change at every size from 48 to 192 vertices and nothing smaller
does, while the release grows with the system. Sealed, the change completes only if the reservoir can absorb
the release, and it leaves one relic defect of fixed size however large the torus. Mapped from $\lambda = 1.05$ to 1.45, the torus is metastable
...
model offers, a torus with one direction curled, opening into the flat torus. It is a decompactification in
...
event a burp. The curled torus exists as a metastable state only for $\lambda > 1$; we map it along
$\lambda$ and follow its decay.
...
cows being unavailable, every space in this paper is a torus.}
\label{fig:tori}
\end{figure*}
...
The mean time to the first exit is therefore an Arrhenius law whose attempt frequencies are counted, not
fitted,
\begin{equation}
...
At $\lambda = 1.25$ the activation energy, the cost of move A, is $\Delta E^\ddagger = 12$. Over twelve conditions ($g = 1.4$ to 2.5, $N = 64$ and 144,
...
release $\varepsilon$ per vertex to within 1\%. The others settle on metastable defected sheets, read from
...
spare the torus never converts, although the flat torus is lower. The activation energy is fixed while the
release, $\varepsilon N$, grows with the system.
...
\emph{One relic, however large.} In the coldest box ($C = 2N$), the number of relic defects per
...
some first exits may recross into the torus instead of growing into a front, a transmission coefficient
$\kappa < 1$ in the language of transition-state theory, and if so $\kappa \approx 0.70$ to 0.87 at
$\lambda = 1.05$. That reading is tested directly by counting exits and recrossings (PREREGISTRATION T22 in
Ref.~\cite{repo}). How completely the change finishes
...
latter two. The burp described here therefore belongs to the family on the $\lambda > 1$ side; the map of
Sec.~\ref{sec:lambda} shows it keeping its character down to $\lambda = 1.05$, where it is cleanest, with a
release $\varepsilon = 4(\lambda - 1)$ that vanishes at the published value. The same
...
\cite{DQM25} weights each state by its number of symmetries (the order of its automorphism group), which is
$2N$ for the curled torus and 2 one
...
showing that a model of emergent geometry contains such a change. We note only that the relic found here, of
fixed size and one per change however large the system, is the kind of object a dark-matter relic of such a
