...
and 96, the first exit arrives when Eq.~(\ref{eq:tau}) says, within one standard error in all six cells. The
same count finds recrossings: about a third of exits return to the torus instead of growing into a front, a
transmission coefficient $\kappa = 0.56$ to 0.68 in the language of transition-state theory, with no trend in
$\lambda$. Recrossings add to the time to convert, not to the time to the first exit, and do not single out
small $\lambda$; we read the excess near $\lambda = 1$ as statistical. How completely the change finishes
