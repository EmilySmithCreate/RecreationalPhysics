...
defect of fixed size however large the torus. Mapped from $\lambda = 1.05$ to 1.45, the torus is metastable
up to 1.35 and the change keeps its two-order, single-front character wherever it is, finishing cleanly near
$\lambda = 1$ and leaving more defects as $\lambda$ grows. The release vanishes at $\lambda = 1$. Every verdict quoted was pre-registered.
\end{abstract}
...
the mean wait at 200 sweeps near $\lambda = 1.33$. Wherever it is stuck, the change keeps the character of
Sec.~III: at half conversion 95 to 100\% of vertices belong to one order or the other, the converted region is
one piece holding 76 to 100\% of it, and the release is exactly $4(\lambda - 1)$ per vertex wherever the flat
...
measured. Near $\lambda = 1$ the measured waits run 15 to 43\% longer than Eq.~(\ref{eq:tau}), which the
formula does not contain. Eq.~(\ref{eq:tau}) gives the time to the \emph{first} exit; when the release is small,
some first exits may fall back into the torus instead of growing into a front, and if so about 70 to 87\% of
first exits go through at $\lambda = 1.05$. That reading is tested directly by counting exits and fall-backs
(PREREGISTRATION T22 in Ref.~\cite{repo}). How completely the change finishes
depends on $\lambda$ [Fig.~\ref{fig:lambda}(b)]: at $\lambda = 1.05$ every decay ends at the flat torus, while the
share ending on a defected sheet rises to 21\% at 1.30 and 54\% at 1.35. At fixed coupling, then, the leftover is
...
\caption{The map along $\lambda$ at $g = 1.5$. (a) Mean waiting time where the torus is stuck, four sizes, with
...
$\lambda > 1$ side; the map of Sec.~\ref{sec:lambda} shows it keeping its character down to $\lambda = 1.05$,
where it is cleanest, with a release that shrinks as $4(\lambda - 1)$ and vanishes at the published value. The same
