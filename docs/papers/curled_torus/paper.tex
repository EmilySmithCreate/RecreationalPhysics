...
term by a coefficient $\lambda$. Only $\lambda = 1$ is CQG; every result below is at $\lambda > 1$, in a
neighbouring family where the energy is no longer a curvature.
...
turning into another. Such changes arise wherever a region of the graph is stuck in one discrete arrangement
while another is favored, as with the allotropes of CQG \cite{T24}. We study the simplest such
...
cows being unavailable, every space in this paper is a torus; still, every bull burps.}
\label{fig:tori}
\end{figure*}
...
showing that a graph energy one coefficient away from a model of emergent geometry contains such a change. We note
two things only. The relic found here is of fixed size and about one per change at every size we reached; it
anneals away at every fixed coupling we tried, survives when the box cools faster than it heals, and grows in
number with the number of seeds that start the change \cite{repo}. And the change needs a push of fixed size while
what it releases grows with the system.
...
files; every result file carries its configuration and software versions. The predictions and analysis rules
behind every verdict quoted were committed before the runs they concern, with every amendment recorded; the two
exploratory tests are labeled where they appear. Code, data, configurations and pre-registrations: \cite{repo}.
...
\bibitem{T25} C.~A. Trugenberger, \emph{Networks as the fundamental constituents of the universe}, J. Phys. Complex. \textbf{6}, 042001 (2025), arXiv:2512.17676.
