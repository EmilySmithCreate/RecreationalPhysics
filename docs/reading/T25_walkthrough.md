...
- **This is no longer a difference** *(your decision, 2026-09-20; VISION Update 10).* His allotropes are *less* stable than the space around them and are left behind by the change; X2 was originally proposed as *more* stable. Claim 5 now allows either direction, requiring only that the change happens, that it releases the energy, and that the leftover balances the books. So his allotropes are a candidate instance of X2 rather than a rival to it, and the leftover may equally be dark energy. The cost of that breadth is that claim 5 now excludes very little, and should not be counted as evidence if it is later satisfied.
