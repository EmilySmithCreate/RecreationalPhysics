...
| 2 | Build Trugenberger's model. Gate: reproduce the published square-density curve [KTB19 Fig. 8, D = 2] | none yet; establishes trust | Kernel built and tested; first look run. **Gate not yet passed.** As of Update 3: model readings Q1–Q3 checked against the sources; [T25] Fig. 3 digitised and compared at N = 160 (exploratory): agreement on the hot side, a jump on the cold side that the capped model does not have. The full Hamiltonian (T2) is needed to go further. The drift with ln N is not contradicted by any published measurement |
