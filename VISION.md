...
- **Resolved, in favour of the local wall** *(added after the Arrhenius measurement, same day; see the next bullet for what was wrong)*. Counting exactly: the cheapest way out of a tube costs 12 and the sampler offers exactly 3 such moves a sweep **at every size**, because a sweep is 2N attempts while the proposals go as N² and the good ones as N. So the waiting time should be 1/(3 exp(−12/g)) sweeps with nothing fitted. ~~Over ten conditions spanning an eighty-five-fold range of waiting times and two sizes, the mean of observed over predicted is 0.997.~~ *Corrected 2026-09-23: the detailed record (ASSUMPTIONS, "The wall measured against temperature") has twelve conditions over a ninety-fold range and two sizes, plain mean of observed over predicted 0.983, weighted mean 0.884 ± 0.060, every ratio between 0.71 and 1.62. The figures struck through came from an earlier reading and reached a draft letter before being caught.* The wall is local, its height is the single move, and a flat waiting time in sweeps is what that predicts rather than evidence against it.
...
Update 22 (2026-09-24): **DECISION (the author's): build the three-dimensional version of the model, six links per point.** Why this page changes: S1 requires a dated decision before any run on a setting this project has not used, and two of the author's positions of this day cannot be tested in two dimensions. Details: `docs/parked/extension_2026-09-22.md` (24 September sections).

- **Why three dimensions.** The author holds that the three space directions are intertwined and curl together, never one alone, and that one activation opens all three. In the 2D model the curled torus has one direction curled and one open, which is the partial state her rule forbids. And the question "does curved space move matter?" may have no answer in two dimensions: in two space dimensions even Einstein's gravity gives no pull between masses at rest (general knowledge, to verify).
- **What it is, and what it is not.** It is the published model at D = 3: [KTB19] runs D = 2 and 3 at N ≈ 200, and [T22] reaches N = 500. The knob λ carries over unchanged. It is **not** a new energy term; it is a published setting we have not built. Every setting run is published, as S1 requires.
