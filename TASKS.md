...
VISION S5: a physicist reads it. Draft a short, honest note to C. Kelly, F. Biancalana or C. Trugenberger once Gates A and B have passed: what was reproduced, what was not, the code link, and a request for a sanity check. The sharpest way to put the disagreement (ASSUMPTIONS, axis section): we match Fig. 3 at both ends, to 0.001 to 0.039 over eleven points from g = 6.3 to 178 and to 0.04 at g = 2, and differ only in between, where its points drop almost vertically at one coupling and ours rise smoothly. **First question to ask: was Fig. 3 run on bipartite graphs?** [KTB19] Sec. 4 says its own were; [T25] says only that one *can* use them. If Fig. 3 allowed triangles and pentagons, that alone could explain everything, since triangles earn 9/8 against a square's 1. Also worth a line each: the 14-vertex baby universe (Q8) and the 30-vertex triangle-and-pentagon piece with H = 0 (Q10).
