...
**Public-facing work.** The plain-language page and a draft post are in `docs/public/`; the draft note to the model's authors, with the addresses found from published sources, is in `docs/outreach/`. Rules for the pages: every claim says whose it is, and no process narration.
