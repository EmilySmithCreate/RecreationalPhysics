...
- Sheet-decay rehearsal (design brief experiment 2; `run_cqg_quench.py` with `"start": "torus"`): below λ = 1 a perfect flat sheet waits, then drops abruptly onto exact ladder levels, first a tube. The wrong direction for claim 4 and at fixed temperature; sealed and leaky runs are the next step on that track.
