...
- External checks at N = 160, exploratory (ASSUMPTIONS section D). **Reproduced:** λ = 1 without the cap matches the digitised [KTB19] Fig. 8a to rms 0.005 over the 14 points where our chain equilibrates (g ≥ 4.9). **Not reproduced:** the cold-side jump of [T25] Fig. 3, by up to 0.41–0.44, with either variant; both of ours are smooth and in equilibrium there (opposite starts agree to 0.003). The two published figures conflict with each other, so this is a question for the authors, not a bug hunt. **Axis re-checked 2026-09-20: [T25] Fig. 3 is log base 10, confirmed three ways** (ASSUMPTIONS, axis section). Put the disagreement precisely: we match Fig. 3 at both ends and differ only for g between 2 and 6.3, where its points drop almost vertically and ours rise smoothly. Leading candidate explanation: [T25] never says Fig. 3 was bipartite, only that one *can* use bipartite graphs; triangles earn 9/8 against a square's 1, so allowing them could steepen the rise. An earlier guess that Fig. 3 shows the uncapped model was tested and withdrawn. No first-order signal at this size in either variant. Do not call any of this a finding.
