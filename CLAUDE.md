...
- Design track (VISION step 5, `docs/design/model_x_brief.md`): the owner wants to get to a designed model of X. Exact facts so far: dimension = number of edge pairs at a vertex that close no square (sheet 2, tube 1, cube 0); for 0 < λ < 1 the mechanism of claim 4 exists but runs from space to cubes; for 1 < λ < 2 exhaustive search at N = 18 finds dips above the flat ground state. A new knob needs a dated VISION decision before any run (S1).
