...
- **O41 In 3D, the fully curled state is stuck for now, but only for 1 < λ < 1.2 (exact, with Q21's energy).**
  (2026-09-24; `scripts/exact_exits_d.py`, tested in `tests/test_exact_exits_d.py` against paper 1's moves A and
  B, O40 and networkx.) Every single move out of each curled torus was listed and priced (−16ΔS + 4λΔX), with
  offers per sweep 4c/(ND²), which is paper 1's count at D = 2.
  - **The ladder is additive:** H/N = 4(λ − 1) per curled direction, in 3D as in 2D. So flat is 0, one curled
    4(λ − 1), two 8(λ − 1), and all three (the 6-cube) 12(λ − 1).
  - **Where each is stuck for now:**
    - One curled (4 × 6 × 6): for 1 < λ < 2. Its cheapest wall is 96 − 48λ (36 at λ = 1.25, three times the 2D
      tube's 12), offered 1.33 times per sweep.
    - Two curled (4 × 4 × 6): for 1 < λ < 1.2. Its wall is 96 − 80λ.
    - All three curled (the 6-cube, alone or as a gas of two): for 1 < λ < 1.2. Its wall is 96 − 80λ (16 as
