...
- **Why the open question is worth the work.** The published case that the geometry-forming transition is continuous rests on one 2019 paper, which states that a finite-size-scaling analysis was "precluded" and that its runs were "some way off the asymptotic regime". That analysis is what separates a weak first-order transition from a continuous one, and nobody has done it. Both published papers also work at a fixed coupling, which cannot represent the coexistence region where a latent heat would show itself. Details in `ASSUMPTIONS.md`, section D.
...
- **Design track:** a brief for a model of X (`docs/design/`), and a first rehearsal in which a perfect flat sheet, stable for now, gives way abruptly in steps that land on exactly predicted energies, the first step a dimension curling up. It runs the wrong way round for the hypothesis (space is what gives way).
- **Two predictions made in advance and then tested.** The waiting time before a curled-up dimension opens out was predicted from a count of the ways over its energy wall, with nothing fitted, and matched over a ninetyfold range of waiting times. And in a sealed system, where the released energy cannot escape, the amount of disorder left behind was predicted from a separately measured equilibrium curve and matched to 0.003. Both are in `ASSUMPTIONS.md`, section D. **The right way round has now been seen as well, at toy size:** at λ = 1.25 a tube, with one large dimension, lasts thousands of sweeps and then opens out abruptly into a connected flat sheet, giving off exactly the energy known in advance. Sixty-four to ninety-six points, four runs each, fixed temperature; the caveats are in `ASSUMPTIONS.md`, section D. **A prediction we then made about the mechanism is not supported:** the waiting time does not fall with size as local nucleation would require, although the wall itself turns out to be local and sharply 12 units at every size (VISION Update 9; the two do not yet fit together). The end state and the energy given off per vertex are unchanged at every size tried.
